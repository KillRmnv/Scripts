...
# Adapted from dusklinux/dusky for JaKooLit/Hyprland-Dots
...
build_menu() {
    ...
    local i item icon display_name prefix suffix
    ...
    for i in "${!SHADERS[@]}"; do
        ...
        if [[ "$item" == "$PREVIEW_SHADER" ]]; then
            ...
            suffix=" (Active)</b>"
            icon="${ICONS[active]}"
            [[ "$item" == "off" ]] && icon="${ICONS[off]}"
        else
            icon="${ICONS[shader]}"
            [[ "$item" == "off" ]] && icon="${ICONS[inactive]}"
        fi

        display_name=$(escape_pango "$item")
        [[ "$item" == "off" ]] && display_name="Turn Off"
    ...
    while true; do
        ...
        case "$exit_code" in
            ...
            11)
                ...
                ;;
            1)
                apply_serialized "$ORIGINAL_SHADER"
                CLEANUP_NEEDED=0
                exit 0
                ;;
            *)
